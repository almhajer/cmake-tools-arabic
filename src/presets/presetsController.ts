...
export class PresetsController implements vscode.Disposable {
    ...
    private async getConfigurePresetTemplateGenerator(): Promise<string | undefined> {
        const cmakeExe = await this.project.getCMakeExecutable();
        return selectConfigurePresetTemplateGenerator({
            platform: process.platform,
            hasNinja: (await paths.which('ninja')) !== null,
            hasNinjaBuild: (await paths.which('ninja-build')) !== null,
            hasMake: (await paths.which('make')) !== null,
            hasMinGWMake: (await paths.which('mingw32-make')) !== null,
            hasNMake: (await paths.which('nmake')) !== null,
            supportsDefaultGenerator: cmakeExe.isDefaultGeneratorSupported ?? false
        });
    }

    private async createConfigurePresetTemplate(displayName: string, cacheVariables: { [key: string]: preset.CacheVarType | undefined }): Promise<preset.ConfigurePreset> {
        const generator = await this.getConfigurePresetTemplateGenerator();
        const description = generator
            ? localize('description.custom.configure.preset.generator', 'Sets the {0} generator, build and install directory', generator)
            : localize('description.custom.configure.preset.default.generator', 'Sets build and install directory');
        ...
        if (!chosenItem) {
            ...
            switch (chosenItem.name) {
                ...
                case SpecialOptions.ToolchainFile: {
                    ...
                    newPreset = await this.createConfigurePresetTemplate(displayName, {
