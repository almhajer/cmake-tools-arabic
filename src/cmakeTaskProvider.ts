...
const buildLog = logging.createLogger('build');
const cmakeLog = logging.createLogger('cmake');
...
export class CustomBuildTaskTerminal extends proc.CommandConsumer implements vscode.Pseudoterminal {
    ...
    private writeInfoLine(line: string): void {
        this.emitLine(line);
        this.getOutputLogger().info(line);
    }

    private writeErrorLine(line: string): void {
        this.emitLine(line);
        this.getOutputLogger().error(line);
    }
    ...
    async open(_initialDimensions: vscode.TerminalDimensions | undefined): Promise<void> {
        ...
        switch (this.command) {
            ...
            case CommandType.cleanRebuild:
                ...
                break;
            default:
                this.writeErrorLine(localize("command.not.recognized", '{0} is not a recognized command.', `"${this.command}"`));
    ...
    private async correctTargets(project: CMakeProject, commandType: CommandType): Promise<string[]> {
        ...
        if (shouldIgnore && targetIsDefined) {
            this.writeInfoLine(localize("target.is.ignored", "The defined targets in this task are being ignored."));
        }
    ...
    private async isTaskCompatibleWithPresets(project: CMakeProject): Promise<boolean> {
        ...
        this.writeErrorLine(localize('task.not.compatible.with.preset.setting', 'The selected task is not compatible with preset setting.'));
        this.closeEmitter.fire(-1);
        return false;
    }
    ...
    private async getProject(): Promise<CMakeProject | undefined> {
        ...
        if (this.workspaceFolder !== undefined) {
            this.writeInfoLine(localize("workspace.is", "Workspace is {0}", this.workspaceFolder.uri.path));
        ...
        if (!project) {
            ...
            this.writeErrorLine(localize("task.failed", "Task failed."));
            this.closeEmitter.fire(-1);
        }
        return project;
    }
    ...
    private async runConfigTask(): Promise<any> {
        this.writeInfoLine(localize("config.started", "Config task started..."));
        ...
        if (cmakeDriver) {
            ...
            if (result === undefined || result === null) {
                this.writeInfoLine(localize('configure.terminated', 'Configure was terminated'));
                this.closeEmitter.fire(-1);
            } else {
                this.writeInfoLine(localize('configure.finished.with.code', 'Configure finished with return code {0}', result.exitCode));
                this.closeEmitter.fire(result.exitCode);
            }
        } else {
            log.debug(localize("cmake.driver.not.found", 'CMake driver not found.'));
            this.writeErrorLine(localize("configure.failed", "Configure failed."));
            this.closeEmitter.fire(-1);
        }
    }
        ...
        if (cmakeDriver) {
            ...
            if (this.preset) {
                ...
                if (!buildPreset) {
                    ...
                    this.writeErrorLine(localize("build.no.preset.failed", "Build preset {0} not found. {1} failed.", this.preset, taskName));
                    if (doCloseEmitter) {
                        this.closeEmitter.fire(-1);
                    }
                    return -1;
            ...
            log.debug(localize("cmake.driver.not.found", 'CMake driver not found.'));
            this.writeErrorLine(localize("build.failed", "{0} failed.", taskName));
            if (doCloseEmitter) {
                this.closeEmitter.fire(-1);
            }
            return -1;
        }
        this.writeInfoLine(localize("build.started", "{0} task started....", taskName));
        this.writeInfoLine(proc.buildCmdStr(cmakePath, args));
        try {
            ...
            this._process = undefined;
            if (result.retc) {
                this.writeErrorLine(localize("build.finished.with.error", "{0} finished with error(s).", taskName));
            } else if (result.stderr || (result.stdout && result.stdout.includes(": warning"))) {
                this.writeInfoLine(localize("build.finished.with.warnings", "{0} finished with warning(s).", taskName));
            } else {
                this.writeInfoLine(localize("build.finished.successfully", "{0} finished successfully.", taskName));
            }
            if (doCloseEmitter) {
    ...
    private async runTestTask(): Promise<any> {
        this.writeInfoLine(localize("test.started", "Test task started..."));
        ...
        if (cmakeDriver) {
            ...
            if (this.preset) {
                ...
                if (!testPreset) {
                    ...
                    this.writeErrorLine(localize("ctest.failed", "Test preset {0} not found. Test failed.", this.preset));
                    this.closeEmitter.fire(-1);
                    return;
                }
            }
            ...
            if (result === undefined) {
                this.writeInfoLine(localize('ctest.run.terminated', 'CTest run was terminated'));
                this.closeEmitter.fire(-1);
            } else {
                this.writeInfoLine(localize('ctest.finished', 'CTest finished'));
                this.closeEmitter.fire(0);
            }
        } else {
            log.debug(localize("cmake.driver.not.found", 'CMake driver not found.'));
            this.writeErrorLine(localize("test.failed", "CTest run failed."));
            this.closeEmitter.fire(-1);
        }
    }

    private async runPackageTask(): Promise<any> {
        this.writeInfoLine(localize("package.started", "Package task started..."));
        ...
        if (cmakeDriver) {
            ...
            if (this.preset) {
                ...
                if (!packagePreset) {
                    ...
                    this.writeErrorLine(localize("cpack.failed", "Package preset {0} not found. CPack failed.", this.preset));
                    this.closeEmitter.fire(-1);
                    return;
                }
            }
            ...
            if (result === undefined) {
                this.writeInfoLine(localize('cpack.run.terminated', 'CPack run was terminated'));
                this.closeEmitter.fire(-1);
            } else {
                this.writeInfoLine(localize('cpack.finished', 'CPack finished'));
                this.closeEmitter.fire(0);
            }
        } else {
            log.debug(localize("cmake.driver.not.found", 'CMake driver not found.'));
            this.writeErrorLine(localize("cpack.failed", "CPack run failed."));
            this.closeEmitter.fire(-1);
        }
    }

    private async runWorkflowTask(): Promise<any> {
        this.writeInfoLine(localize("workflow.started", "Workflow task started..."));
        ...
        if (cmakeDriver) {
            ...
            if (this.preset) {
                ...
                if (!workflowPreset) {
                    ...
                    this.writeErrorLine(localize("workflow.failed", "Workflow preset {0} not found. Workflow failed.", this.preset));
                    this.closeEmitter.fire(-1);
                    return;
                }
            }
            ...
            if (result === undefined) {
                this.writeInfoLine(localize('workflow.run.terminated', 'Workflow run was terminated'));
                this.closeEmitter.fire(-1);
            } else {
                this.writeInfoLine(localize('workflow.finished', 'Workflow finished'));
                this.closeEmitter.fire(0);
            }
        } else {
            log.debug(localize("cmake.driver.not.found", 'CMake driver not found.'));
            this.writeErrorLine(localize("workflow.failed", "Workflow run failed."));
            this.closeEmitter.fire(-1);
        }
    }
